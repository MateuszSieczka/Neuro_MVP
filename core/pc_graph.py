...
def _node_cone(
    feedforward_nodes: tuple, edges: tuple, n_nodes: int,
) -> dict:
    """Per-free-node plan for assembling its block free-energy Hessian.

    Every freely-inferred node steps by its **block** free-energy Hessian, the
    natural (Fisher) metric ``H_j = (leak+Π_j)·I + Σ_leaves Jᵀ(Π⊙hold)J`` of
    the generative model — of which the diagonal-Newton curvature ``L_j`` used
    by the cheap step is the one-hop diagonal, and the *precision-dominated
    limit*.  ``J = ∂ŷ/∂μ_j`` is the Jacobian of every likelihood prediction
    reachable from ``j`` through the *feedforward* recognition path (a free
    descendant is held fixed within the coordinate step — it co-relaxes, the
    envelope theorem — so the derivative flows only through deterministic
    feedforward nodes, exactly as error/curvature relay does).  ``H`` is
    assembled by one forward pass that pushes the ``(size × d)`` tangent
    ``Jpred = ∂ŷ/∂μ_j`` down that path — this routine returns the static
    traversal plan for every non-feedforward node so the JIT'd relaxation never
    re-derives the topology.  Which nodes actually *use* the full solve rather
    than the diagonal limit is decided at runtime (own precision vs child-relay
    curvature), not by a static tag — so a flat-prior efferent node inverts by
    the full metric while a sharp perceptual node stays diagonal-cheap.

    For each node ``a`` the plan is ``(interior, leaves)``:
    ...
    * ``leaves`` — the *free* nodes the cone reaches (the likelihood
    ...
    source is the root or an interior node — so a node with parents outside the
    cone (held fixed) contributes no spurious tangent.
    """
    ff = set(int(j) for j in feedforward_nodes)
    ...
    for a in range(n_nodes):
        if a in ff:
            continue                             # feedforward nodes are not stepped
        # Discover the cone: walk out of the root and out of interior
...
def init_pc_graph_params(
    ...
    *,
    ...
    # Pre-compute the block-Hessian cone of every free node (the feedforward
    # sub-graph its Gauss–Newton Jacobian threads through) once, so the
    # relaxation never re-derives it; validates the cones are well-formed up
    # front.  A flat-prior efferent node (small Π) automatically takes the full
    # solve and a sharp perceptual node the diagonal limit — chosen at runtime
    # from the node's precision, not a static tag.
    _node_cone(feedforward_nodes, edges, len(sizes))
...
def _graph_relax_step(
    ...
    **The metric is one rule, not two.**  ``L_j`` above is the *diagonal* of
    the block free-energy Hessian

        H_j = (leak + Π_j)·I  +  Σ_{cone leaves c} Jc_jᵀ (Π_c ⊙ hold_c) Jc_j

    the natural (Fisher) metric of the generative model, assembled from the
    prediction-Jacobians pushed down node ``j``'s feedforward cone
    (``node_cone``, via :func:`_node_gn`).  Every free node's step is
    ``Δμ_j = η_μ · H_j⁺ g_j``, of which the cheap ``g_j/L_j`` divide is the
    **precision-dominated limit**: when the node's own precision ``Π_j``
    dominates the child-relayed curvature, ``H_j ≈ diag(Π_j)`` and the solve is
    exactly the divide.  So which nodes take the full solve is decided *at
    runtime* — a dimensionless comparison of ``Π_j`` against the child-relay
    curvature norm (a Gershgorin bound of ``H_j``'s off-prior part) — not by a
    static tag.  A sharp perceptual node (large ``Π_j``) stays on the diagonal;
    a flat-prior *efferent* node (small ``Π_j``, e.g. a motor command carrying
    only a weak effort prior) has its metric set entirely by the child relay,
    where the diagonal is a degenerate metric — its step scales as ``ε/‖W‖²``
    and freezes as the forward model sharpens — so it automatically takes the
    full ``H_j⁺ g_j`` solve.  ``H_j⁺`` (the pseudo-inverse) gives the
    least-squares command in the forward model's range and the minimum-norm
    command in its null space (the correct inversion of a flat prior), and is
    invariant to the forward-model gain: a sharper model no longer freezes the
    command.  This is the concrete sense in which **Π self-arbitrates the
    inference metric**.
    """
    act = params.act
    N = params.n_nodes
    ...
    def _node_gn(j: int) -> tuple[Array, Array]:
        """Block Gauss–Newton ``(H, g_child)`` of free node ``j`` over its cone.
        ...
        (envelope theorem).  This is what makes a flat-prior node's step an
        inversion of the *goal* rather than a compromise with the command's own
        free consequences (proprioceptive reafference, efference copy) — which,
        mixed into an unmasked gradient, decorrelate the command from the true
        inverse entirely.  ``g_child`` is ``∂(child free energy)/∂μ_j``; the
        caller adds the own-prior term.  The ``(leak+Π_j)·I`` diagonal is the
        node's own prior precision (for an efferent node, the weak effort/cost
        prior — Todorov 2004; Friston 2010): negligible against the goal
        curvature once the forward model has any gain, but it keeps ``H``
    ...
    # Update every free node by the block free-energy Hessian; the cheap
    # diagonal-Newton divide is its precision-dominated limit, selected per node
    # at runtime (own precision vs child-relay curvature) — no static action
    # tag.  Both forms are assembled and one is selected: on these node sizes
    # the dense solve is negligible, and computing both keeps the JIT sweep
    # branch-free.  A large-node build could gate the solve on the *same*
    # criterion (evaluate the diagonal alone where it dominates) — the criterion
    # is identical, only the evaluation is lazier.
    new_mu = list(mu)
    for j in range(N):
        if j in ff_set:
            continue                             # already set to its prediction
        # Diagonal-Newton curvature/gradient (the precision-dominated limit):
        # own error propagates up (g) and each child's curvature adds to L (a
        # feedforward child contributes its relayed curv_eff, a free child its Π).
        g = xi[j]
        L = pi[j]
        if outgoing[j]:
            acc = jnp.zeros_like(mu[j])
        ...
        L = L + params.leak
        diag_upd = mu[j] - params.eta_mu * g / (L + tiny)
        # Full block Gauss–Newton over the node's feedforward cone (the natural
        # metric): scale-covariant minimum-norm solve ``Δμ = η_μ·H⁺ g``, with
        # ``H`` and ``g`` from the same linearisation and the child gradient
        # restricted to held (goal/observation) leaves (a free descendant
        # co-relaxes and drops out — envelope theorem).  Scaled by the same η_μ
        # as every node: the graph relaxes all free nodes together (Jacobi), so
        # a uniform sub-unit step keeps a node in lock-step with its co-relaxing
        # children; a full Newton jump on one coordinate desynchronises them.
        H, g_child = _node_gn(j)
        g_full = xi[j] + params.leak * mu[j] + g_child
        full_upd = mu[j] - params.eta_mu * (jnp.linalg.pinv(H) @ g_full)
        # Runtime metric criterion (dimensionless, un-tuned): own prior precision
        # vs the child-relay curvature norm (Gershgorin bound of H's off-prior
        # part).  Own precision dominates ⇒ H ≈ diag(Π_j), solve ≡ divide, stay
        # on the cheap diagonal; else the child relay sets the metric (a
        # flat-prior efferent node) and the full solve is taken.
        diag_floor = params.leak + pi[j]
        relay = H - jnp.diag(diag_floor)
        relay_norm = jnp.max(jnp.sum(jnp.abs(relay), axis=1))
        use_diag = jnp.min(diag_floor) >= relay_norm
        updated = jnp.where(use_diag, diag_upd, full_upd)
...
def pc_graph_relax(
    ...
    *subset* of a node's dimensions (a partial preference or partial
    ...
    node_cone = _node_cone(params.feedforward_nodes, params.edges, params.n_nodes)
    ...
    def body(_, mu):
        return _graph_relax_step(
            ...
            ff_set, ff_fwd_order, ff_rev_order, node_cone,
        )
...
def init_region_graph(
    ...
    *frozen* random granule expansion (:func:`apply_granule_expansion_init`,
    ...
    params = init_pc_graph_params(
        ...
        # PCGraphParams.fixed_pi_nodes / core.pc_active.set_action_prior).  Its
        # inference metric is *not* tagged: carrying only a weak effort prior
        # (small Π), the relaxation's runtime criterion routes it to the full
        # block Gauss–Newton solve automatically — the diagonal metric of a
        # low-D flat-prior command fanning into the sharp
        # motor→cerebellum→sensory forward model is degenerate and freezes the
        # command as the model sharpens (see _graph_relax_step / _node_cone).
